...
function monthlyAmortizationSchedule(P, r, n, M, i) {
    ...
    if (i === 1) { // Check if it's the first iteration
        ...
        let headerCell1 = headerRow.insertCell(0); // Create a new cell in the row
        headerCell1.innerHTML = "<th>Yearly Interest Total</th>"; // Set the content of the cell to the header
        let headerCell2 = headerRow.insertCell(1); // Create a new cell in the row
    ...
    if (P > 0 || n > 0) { // Check if the principal or the number of payments is greater than 0
        ...
        if (i % 12 === 0) { // Check if a year has passed
            ...
            cell1.textContent = "Year " + (i / 12) + ": " + "$" + yearlyInterestTotal.toFixed(2); // Set the content of the cell to the yearly interest total
...
function outputCalResult (){
    ...
    //Declare, initialize and calculate total interest paid and total payment
